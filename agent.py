...
class Agent:
    ...
    def critic_learn(self, batch_size, retain_graph=True, gpu_list=''):
        """当前评论网络参数更新"""
        if len(self.experience_playback) < batch_size:
            batch_size = len(self.experience_playback)
    ...
    def actor_learn(self, batch_size, retain_graph=True, gpu_list=''):
        """actor当前网络参数更新"""
        ep_state_b = torch.cat(tuple(self.experience_playback['state'][-batch_size:]), 0)
        ep_action_b = torch.cat(tuple(self.experience_playback['action'][-batch_size:]), 0)
        ep_state_action_b = torch.cat((ep_state_b, ep_action_b), 1)
        ...
        actor_loss = self.actor_net_loss(critic_q, batch_size_tensor)
        actor_loss.backward(retain_graph=retain_graph)
        self.actor_optimizer.step()

    @staticmethod
    def state2actor_tensor(state):
        l_p = state[1]
        cm = state[2]
        cm_w = state[3]
        x = []
